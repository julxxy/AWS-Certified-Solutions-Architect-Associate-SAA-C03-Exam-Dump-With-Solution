...
| 新增         | 说明                                                              |
|--------------|-------------------------------------------------------------------|
| **中文译文** | 解析全文已译（`AWS SAA-03 Solution.zh-CN.txt`）；选项译文已补齐   |
...
```bash
./deploy.sh # 拉最新代码 + 按需重建 + 自检 + 幂等启动（./deploy.sh --help 看细节）
```
...
- **可出题 683 / 684** —— 其余 1 题答案尚未确定或存在歧义，已标记 `needs_review` 并排除出考试池
- **170 题没有解析** —— 源文件里 51–99 等整段区间只有题目和答案。这些题答案可信， 照常出题，只是答完没有解析可看
