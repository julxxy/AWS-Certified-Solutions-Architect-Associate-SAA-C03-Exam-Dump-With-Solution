...
>    改行为之前先看它，别从代码反推。
> 2. **验收基准** —— §6。其中数据侧的检查已由 `python3 scripts/verify_bank.py` 自动化，
>    行为侧的仍需人工过一遍。
...
你是一名资深 Python 工程师。在仓库 `AWS-Certified-Solutions-Architect-Associate-SAA-C03-Exam-Dump-With-Solution`
中，构建一个 **本地运行、离线可用、中英对照**的 AWS SAA-C03 刷题系统。

交付两样东西：

1. **题库构建脚本**（两阶段，见 §3）—— 先从 PDF 抽出英文题库并生成待译清单，再把 PDF + txt + 两路中文译文合并成结构化 JSON 题库。
2. **本地网页版刷题程序** —— `python3 scripts/app.py` 启动，浏览器做题，支持「模拟考试」与「滚动学习」两种模式，进度落盘到 `data/`。

**最高优先级：程序必须能真正跑起来。** 数据脏、题目缺失是既定事实（见 §2），遇到脏数据要标记并跳过，绝不能因此让程序报错或停止交付。

**边界说明：翻译本身不是你的活。** 选项中文由另一个进程产出。你要做的是：① 抽取 PDF 生成待译清单 `data/i18n_todo.jsonl`；② 读取翻译进程回填的
`data/i18n_zh.jsonl` 并合并。 **任何情况下都不得自己编造或"顺手翻一下"中文** —— 缺译文就留 `null`，由 UI 显示角标。
...
| 启动     | `python3 scripts/app.py` → 监听 `127.0.0.1:8765` → 自动打开浏览器                                                                                                              |
...
| 数据源                          | 角色                        | 关键事实                                                                                                      |
|---------------------------------|-----------------------------|---------------------------------------------------------------------------------------------------------------|
| `AWS Certified ... SAA-C03.pdf` | 题干 + A–F 选项（唯一权威） | 684 题连续无缺；选项 **2832** 个（ABCD 598 / ABCDE 76 / ABCDEF 10，其中 477 题 4 个是图片无文字）；多选 86 题 |
...
1. **PDF 要自己解码** —— `pdftotext` 本机没装也不装。Type3 子集字体 + 十六进制字形码， 统一 `+0x1C` 偏移回 ASCII，另有 6 个连字码位要查表还原。
2. **切分正则不能"优化"** —— 主通道必须要求题号单调递增（否则解析里的编号列表会 把后续几十题整体错位）；补漏只能用定点修复通道。
3. **第 315 题在 txt 里被误编号为 `215]`**，必须特判。
4. **191–200 在 txt 里根本不存在**，不能从该源文件提取答案和解析；可根据原题和官方资料通过人工修正补齐。
...
3. **确定正确答案字母**，按以下优先级：
    ...
    - c. **文本相似度兜底**（关键，覆盖 §2 提到的那 36 题）：把 txt 中的答案句与 PDF 的每个选项做 `difflib.SequenceMatcher` 比对，取最高分； **≥
      0.65 才采纳**，否则判为未确定
...
**191–200 的处理**：这 10 题 PDF 里有完整题干与选项，但源 txt 缺少答案与解析。未经核验时保留 `needs_review: true`、`answer: []`、
`answer_source: "unresolved"`，排除出考试池与滚动学习出题池，但 **保留在题库里**。通过 `data/manual_fixes.json` 补齐并核验的题可正常出题；
有歧义的题可在人工修正中显式设置 `needs_review: true` 和 `review_reason`，保留分析但继续排除。不能因为补了解析就把歧义题放行。
...
```json
{
  ...
  "options": [
    {
      "letter": "A",
      "text_en": "Deploy the applications in eu-central-1...",
      "text_zh": "在 eu-central-1 部署应用程序..."
    },
    {
      "letter": "B",
      "text_en": "Deploy the applications in AWS Local Zones...",
      "text_zh": "通过将公司的 VPC 从 eu-central-1 扩展到所选的 Local Zone..."
    }
  ],
  "answer": [
    "B"
...
- `needs_review: true` 的条件： **只看答案可不可信** —— 答案未确定或有歧义 / 答案个数与 Choose N 不符 / txt 缺该题且尚未人工核验 /
  选项是图片无文字。置位后该题被排除出考试池与学习出题池。
    - ⚠️ **原始规格曾把「解析长度 < 120 字符」也列为条件，实现时推翻了。** 照那样做
      `needs_review:false` 只剩 **430** 题，远低于 §6 的 620 门槛 —— 因为源文件里 51–99 等 整段区间本就只有题目和答案、没有解析（实测 254
      题）。一道答案确凿、只是没解析的题 照样能考，不该踢出题库。
- `explanation_quality`: `ok`（≥120 字符）| `thin`（40–120）| `none`（<40）。 **非阻塞**，只进报告供人工按需补充，不影响能否出题。
...
- **译文覆盖率，拆成三项分别统计**（不要只给一个总数）：
    - 题干：`已译题数 / 684`
    - **选项：`已译选项数 / 可译选项数`，验收门槛见 §6**
      （题库共 2832 个选项；第 477 题的 4 个图片选项已人工转录并补译，当前可译分母为 2832。尚未转录的图片空选项不计入可译分母。）
    - 解析：`已译题数 / 有解析的题数`
...
**硬性约束（中英对照下最容易出的 bug）**：乱序 **只允许对 `options` 数组整体做一次 `random.shuffle`**，让 `text_en` 与 `text_zh` 作为同一个对象一起搬运。
**禁止**对任何单语字段单独排序或分别洗牌——否则会出现「英文显示 A 的内容、中文显示 C 的内容」，而双语并排时两边看起来都像正常选项，极难发现。
...
- **注意 box 1 的间隔是 1 天而不是 0 天**：同一天内重复刷同一题收益很低，隔夜的第一次复习性价比最高。当次会话内的巩固由 Cool-down 段负责，不占用
  box 调度
- 到期题按此优先级排序进入热身段：
    1. 错 + 有把握（高信心错误）
    2. 「假掌握」标记（对 + 蒙的）
...
> **数据侧的检查已经自动化**：`python3 scripts/verify_bank.py`（结构完整性、抽取质量、题型分布、
> 已知脏数据标记、与上次基线的回归对比）。数据源更新后跑它就够，不用手工过下面的清单。
>
> 下面标 🤖 的项目由 `verify_bank.py` 覆盖，标 👤 的仍需人工在浏览器里过一遍。

- [ ] 🤖 `python3 scripts/build_bank.py --extract` 跑通，`data/questions_en.json` 含 684 条，`data/i18n_todo.jsonl` 已生成
- [ ] 🤖 合并修正后的选项总数为 **2832**（其中 477 题 4 个图片选项已人工转录），选项数分布为 ABCD 598 / ABCDE 76 / ABCDEF 10
  （PDF 把 125 题第 5 个选项的标号也印成了 `D.`，抽取时并进 D；由 `data/manual_fixes.json` 拆回 D/E，
  故比 PDF 原始抽取多 1 个选项、ABCD 少一题 ABCDE 多一题）
- [ ] 🤖 抽取文本中 **没有断字**（全文搜 `les`、`traÊc`、`Éow` 等应无命中，`configure`/`traffic`/`flow` 拼写完整）
- [ ] 🤖 `python3 scripts/build_bank.py` 一次跑通，`data/questions.json` 含 684 条记录
- [ ] 🤖 `needs_review: false` 的题数 **≥ 620**（低于此数说明匹配算法有问题，需改进）
- [ ] 🤖 `data/build_report.md` 已生成，逐条列出待人工核对的题号
- [ ] 🤖 抽查题号 **1 / 51 / 253 / 315 / 684**：题干、选项数、正确答案、解析是否正确对应（253 与 315 属已知脏数据，应被正确标记； **315 需从 txt
  中误编号的 `215]` 正确取到**，且真正的 215 未被覆盖）
- [ ] 🤖 191–200 有完整双语题面与解析、人工参考答案且可出题；193 按节点型缓存的比较口径取 B，解析保留部署类型差异说明
- [ ] 🤖 第 477 题含完整中英文题干策略和 A–D 选项，保留 C 的 `bucket-name*` 与 D 的 `bucket-name/*`，答案为 D，且可进入出题池
- [ ] 🤖 多选题（如 18、44、51）`type == "multi"` 且 `answer` 长度与 `Choose two/three` 一致
- [ ] 👤 `python3 scripts/app.py` 启动无报错，浏览器自动打开
- [ ] 👤 断网状态下页面完整可用（无任何外链资源）
...
- [ ] 🤖 `stem_zh_source` 中约 46 题为 `pdf_translation`，其余 638 题为 `solution_paraphrase`
  （补译覆盖到 100% 后，中文解析里切不出题干段的题改由 i18n_zh.jsonl 的 `field:"stem"` 兜底，该来源题数从 26 增至 46）
