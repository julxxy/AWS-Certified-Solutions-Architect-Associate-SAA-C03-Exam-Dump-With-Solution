...
**191–200 的处理**：这 10 题 PDF 里有完整题干与选项（翻译后也会有完整中文），但永远没有答案和解析。一律 `needs_review: true`、`answer: []`、
`answer_source: "unresolved"`，排除出考试池与滚动学习出题池，但 **保留在题库里**，允许在「仅浏览」入口下阅读。不要直接丢弃。
...
- `needs_review: true` 的条件： **只看答案可不可信** —— 答案未确定 / 答案个数与 Choose N 不符 / txt 缺该题 / 选项是图片无文字。置位后该题被排除出考试池与学习出题池。
...
- **译文覆盖率，拆成三项分别统计**（不要只给一个总数）：
    ...
    - **选项：`已译选项数 / 2827`　← 这是本次的主要缺口，验收门槛见 §6**
      （题库共 2832 个选项，其中 477 题的 4 个选项在 PDF 里是图片、无文字可译，不计入分母，故分母为 2828）
    - 解析：`已译题数 / 有解析的题数`
...
- [ ] 🤖 抽取出的选项总数为 **2832**（其中 477 题 4 个为图片型无文字），选项数分布为 ABCD 598 / ABCDE 76 / ABCDEF 10
